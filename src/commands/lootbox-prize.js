...
async function handlePrize(interaction) {
  ...
  switch (sub) {
    ...
    case 'remove-role': {
      ...
      if (removed) {
        await interaction.reply({ content: `✅ Removed role prize **${role.name}**.`, ephemeral: true });
      } else {
        await interaction.reply({ content: `⚠️ Role **${role.name}** was not a configured prize.`, ephemeral: true });
      }
      break;
    }
    ...
    case 'list': {
      ...
      if (prizes.length === 0) {
        await interaction.reply({ content: '📭 No role prizes configured.', ephemeral: true });
        return;
      }
